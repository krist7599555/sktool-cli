...
const { target_directory, selected_commands } = await inquirer.prompt([
  ...
  },
  {
    ...
    choices: COMMANDS,
    default: COMMANDS.filter(it => it != "icon"),
  },
]);
...
if (selected_commands.includes('sveltekit-svg')) {
  await $`pnpm i @poppanator/sveltekit-svg -D`;
  await fs
    .readFile('./vite.config.ts', { encoding: 'utf-8' })
    .then(text => `import svg from '@poppanator/sveltekit-svg';\n` + text)
    .then(text =>
      text.replace(
        `sveltekit()`,
        `sveltekit(),
